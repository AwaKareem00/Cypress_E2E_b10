...
describe('Assertions', () => {
  ...
  it('each() - interacting with multiple web elements', () => {
    ...
    cy.get('#languages_heading').each(($el, index) => {
      cy.log(index)
      cy.wrap($el).should('have.text', 'Programming Languages')
        .and('be.visible')
    })

    cy.get('#tools_heading').each(($el, index) => {
      cy.log(index)
      cy.wrap($el).should('have.text', 'Automation Tools')
        .and('be.visible')
    })
  ...
  it('Assertion Practices', () => {
    ...
    const drop = ['Microsoft', 'Apple']

    cy.get('[id^="company_dropdown"]').each(($el, index) => {
      cy.wrap($el)
        .should('be.enabled')
        .select(drop[index])
        .should('have.value', drop[index])
    })
