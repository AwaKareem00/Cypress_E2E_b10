...
describe('Locating Web Elements', () => {
  ...
  it('Test Case 10 - Validate the form submission', () => {
    cy.get(':nth-child(1) > .control > .input').type('Kareem')
    cy.get('.mr-1').eq(0).click()
    cy.get(':nth-child(3) > .control > .input').type('1234 w 79th st chicago illinois')
    cy.get(':nth-child(4) > .control > .input').type('JohnDoe@gmail.com')
    cy.get(':nth-child(5) > .control > .input').type('(123)234-5678')
